...
t = linspace(1/16000,StimulusDur,StimulusDur*16000);
audioSignal = .5*sin(2*pi*t*1000);
...
while (UD.stop == 0) & tr < UD.totalTrials
    ...
    if tempVar(1) == 1 || tempVar(1) == 3   % scenario I (first one is Pert) : % 40% of trials
        offline_pitch_JND_passive (audioSignal,.01,Gender,StimulusDur,riseTime,fallTime,baseTime);
        %         pause(baseTime);
    ...
    elseif tempVar(1) == 2 || tempVar(1) == 4 % scenario II (first one is no Pert) : % 40% of trials
        ...
        %         pause(baseTime);
        offline_pitch_JND_passive (audioSignal,.01,Gender,StimulusDur,riseTime,fallTime,0);
        conVar = 1;
