...
numlines=1;
defaultanswer={'3','15','15','15', '15', '100'};
answer=inputdlg(prompt,name,numlines,defaultanswer);
...
%% load offline wav file
[w, fs] = audioread('200 hz.wav');
...
Signal =resample(Signal, fs*downFact, fs);
sigInCell = makecell(Signal, frameLen);

AudapterIO('init', params); %needed to save the data

 Audapter('runFrame', sigInCell);


 %%%what????
 
 [w, fs] = audioread('200 hz.wav');
w = resample(w, 48000, fs);
fs = 16000;
wnew=resample(w,16000*3,fs);
signal = makecell(w2, frameLen)
signal = makecell(wnew, frameLen)
Audapter('runFrame', signal);
for n = 1 : length(sigInCell)
Audapter('runFrame', sigInCell{n});
end
for n = 1 : length(signal)
Audapter('runFrame', signal{n});
end

%%%
% load('c:\speechres\audapter_matlab\example_data\trial-1-2.mat')
% sigIn = data.signalIn;
...
%%
ostFN = '../example_data/online_pitch_adaptation.ost'; %opens up a file in "example data" saying there will be one rule
pcfFN = '../example_data/online_pitch_adaptation.pcf'; %sets perturbation to zero, will be written over each trial
